...
class Creator:
    def __init__(self, CONFIG_path, surf_index, FIELD_path=None):
        self.CONFIG_path = CONFIG_path
        self.FIELD_path = FIELD_path
        self.surf_index = surf_index

    cutoff_dict = {'001': 15, '012': 14, '113': 12, '113_unopt': 12}
    ...
    def __make_CONFIG(self, folder_path, atoms_for_repl=None, atoms_names=None,
        ...
        atoms_numbers = {'Al': 13, 'O': 8, 'O_shl': 8, 'Mg': 12, 'Co': 27, 'Fe': 26, 'Cd': 28,
                         'Ca': 20, 'Sr': 38, 'Ba': 56, 'Cr': 24, 'Ga': 31, 'Sc': 21, 'In': 49,
                         'Yb': 70, 'Y': 39, 'Sm': 62, 'La': 57}
    ...
    def __make_CONTROL(self, folder_path, cutoff, traj_step, zero=False):
        ...
        OUTPUT.write(f'delr width       {cutoff}\n')
        OUTPUT.write(f'rvdw cutoff      {cutoff}\n')
    ...
    def fractional_coverage(self, impurities_list, potentials, masses,
                            coverages, atoms_for_replacement, copies_range, zero=False):
        for imp, pot, mass in zip(impurities_list, potentials, masses):
            for coverage in coverages:
                for copy in range(copies_range[0], copies_range[1]):
                    ...
                    self.__make_CONTROL(folder_path, self.cutoff_dict[self.surf_index], 50, zero)
    ...
    def bulk_state_for_one_imp(self, impurities_list, potentials, masses,
        ...
        for imp, pot, mass in zip(impurities_list, potentials, masses):
            ...
            unique_imps, len_imps, len_Al, len_O = self.__make_CONFIG(folder_path, [atom_for_replacement],
                                                                      ['Al'], [imp])
            self.__make_FIELD(folder_path, unique_imps, len_imps, [pot], [mass], [3], len_Al, len_O)
            self.__make_CONTROL(folder_path, self.cutoff_dict[self.surf_index], 50)
    ...
    def make_depth(self, impurities_list, potentials, masses, atoms_for_replacement):
        for imp, pot, mass in zip(impurities_list, potentials, masses):
            for Al in atoms_for_replacement:
                folder_path = f'{self.surf_index}/{imp}_depth/{Al}'
                unique_imps, len_imps, len_Al, len_O = self.__make_CONFIG(folder_path, [Al], ['Al'], [imp])
                self.__make_FIELD(folder_path, unique_imps, len_imps, [pot], [mass], [3], len_Al, len_O)
                self.__make_CONTROL(folder_path, self.cutoff_dict[self.surf_index], 50)
